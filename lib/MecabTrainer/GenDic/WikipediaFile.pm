...
my @SKIP_WORDS = map {
    ...
    # ",「」,空白から始まる、または含むものは単純な固有名詞ではないことが多い(固有名詞だとしてもどうせ表記ぶれで取りこぼす可能性大)ので除外。
    '[\"「\s,]',


    ## TODO:ナカグロ「・」が入っているものはナカグロ入りのものとなしのものと両方辞書登録した方がいいのでは？
);
sub skip_word {
    my ($self, $word) = @_;

    return 1 if length($word) < 2; # 長さ1のエントリも除外
